...
export async function scrapeComponentData() {
  ...
  for (const entry of jsonData) {
    ...
    try {
      ...
      // Find the target div with class "prose" and extract h1 tag
      const targetDiv = document.querySelector("div.prose");
      ...
      if (targetDiv) {
        ...
        // Since there's no <p> tag in the given HTML, we'll assign an empty string
        const cleanPText = ""; // Assign empty string for p
        ...
        console.error(`Div with class "prose" not found on ${href}`);
      }
    } catch (error: any) {
      console.error(
